...
function data_history(fid,hist)
	...
	if (isempty(hist.views)) hist.views=0; end
    fwrite(fid, int32(hist.views(1)),      'int32');
	if (isempty(hist.vols_added)) hist.vols_added=0; end	
    fwrite(fid, int32(hist.vols_added(1)), 'int32');
	if (isempty(hist.start_field)) hist.start_field=0; end	
    fwrite(fid, int32(hist.start_field(1)),'int32');
	if (isempty(hist.field_skip)) hist.field_skip=0; end
    fwrite(fid, int32(hist.field_skip(1)), 'int32');
	if (isempty(hist.omax)) hist.omax=0; end	
    fwrite(fid, int32(hist.omax(1)),       'int32');
	if (isempty(hist.omin)) hist.omin=0; end	
    fwrite(fid, int32(hist.omin(1)),       'int32');
	if (isempty(hist.smax)) hist.smax=0; end	
    fwrite(fid, int32(hist.smax(1)),       'int32');	
    if (isempty(hist.smin)) hist.smin=0; end    
    fwrite(fid, int32(hist.smin(1)),       'int32');    
return
