...
%   avw.hdr.hist.orient = uint8(0);
    ...
    FrameDim  = double(avw.hdr.dime.dim(5)); % frames
    ...
    x = 1:PixelDim;
    for frame = 1:FrameDim,
        for z = 1:SliceDim,
            for y = 1:RowDim,
                fwrite(fid,avw.img(x,y,z,frame),precision);
            end
        end
    end
...
% Update the header
avw.hdr.dime.dim(2:5) = int16([PixelDim,RowDim,SliceDim,FrameDim]);
